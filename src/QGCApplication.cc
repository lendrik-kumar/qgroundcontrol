...
QGCApplication::QGCApplication(
    int &argc, char *argv[],
    const QGCCommandLineParser::CommandLineParseResult &cli)
    : QGuiApplication(argc, argv), _runningUnitTests(cli.runningUnitTests),
      _simpleBootTest(cli.simpleBootTest), _fakeMobile(cli.fakeMobile),
      _logOutput(cli.logOutput), _systemId(cli.systemId.value_or(0)) {
  _msecsElapsedTime.start();

  // Setup for network proxy support
  QGCNetworkHelper::initializeProxySupport();

  bool fClearSettingsOptions =
      cli.clearSettingsOptions;            // Clear stored settings
  ...
  (void)connect(&_missingParamsDelayedDisplayTimer, &QTimer::timeout, this,
  ...
  if (_runningUnitTests || _simpleBootTest) {
    ...
    // settings every time. Include test name or PID to prevent settings file
    // conflicts when tests run in parallel
    if (!cli.unitTests.isEmpty()) {
      applicationName = QStringLiteral("%1_unittest_%2")
                            .arg(QGC_APP_NAME, cli.unitTests.first());
    } else {
      applicationName = QStringLiteral("%1_unittest_%2")
                            .arg(QGC_APP_NAME)
  ...
  fClearSettingsOptions |=
      settings.value(AppSettings::clearSettingsNextBootKey, false).toBool();
...
void QGCApplication::setLanguage() {
  ...
  qCDebug(QGCApplicationLog) << "System reported locale:" << _locale << "; Name"
                             << _locale.name() << "; Preffered (used in maps): "
  ...
  if (_locale.name() != "en_US") {
    ...
    if (JsonParsing::translator()->load(_locale, QLatin1String("qgc_json_"), "",
                                        ":/i18n")) {
...
void QGCApplication::init() {
  ...
  if (QFontDatabase::addApplicationFont(":/fonts/inter") < 0) {
    qCWarning(QGCApplicationLog) << "Could not load /fonts/inter font";
  }

  if (QFontDatabase::addApplicationFont(":/fonts/inter-semibold") < 0) {
    qCWarning(QGCApplicationLog) << "Could not load /fonts/inter-semibold font";
  }

  if (_simpleBootTest) {
    // Since GStream builds are so problematic we initialize video during the
    // simple boot test to make sure it works and verfies plugin availability.
...
bool QGCApplication::_initVideo() {
  ...
  QGCCorePlugin::instance(); // CorePlugin must be initialized before
                             // VideoManager for Video Cleanup
  VideoManager *videoManager = VideoManager::instance();
  videoManager->startGStreamerInit();
  const bool initSucceeded =
      !_simpleBootTest || videoManager->waitForGStreamerInit();
...
void QGCApplication::_initForNormalAppBoot() {
  ...
  // Must register before createRootWindow — root QML references QGCColoredImage
  // which resolves image://coloredsvg/... at load time.
  _qmlAppEngine->addImageProvider(_qgcImageProviderId, new QGCImageProvider());
  _qmlAppEngine->addImageProvider(
      QLatin1String(ColoredSvgImageProvider::ProviderId),
      new ColoredSvgImageProvider());

  QGCCorePlugin::instance()->createRootWindow(_qmlAppEngine);

  AudioOutput::instance()->init(
      SettingsManager::instance()->appSettings()->audioVolume(),
  ...
  windowIcon = _qmlAppEngine->interceptUrl(
      windowIcon, QQmlAbstractUrlInterceptor::UrlString);
    ...
    if (permFile.open(QIODevice::ReadOnly)) {
      while (!permFile.atEnd()) {
        ...
        if (line.contains("dialout") && !line.contains(getenv("USER"))) {
          permFile.close();
          showAppMessage(tr("The current user does not have the correct "
...
void QGCApplication::_missingParamsDisplay() {
  ...
  for (QPair<int, QString> &missingParam : _missingParams) {
    const QString param = QStringLiteral("%1:%2")
  ...
  showAppMessage(tr("Parameters are missing from firmware. You may be running "
                    "a version of firmware which is not fully supported or "
...
void QGCApplication::showAppMessage(const QString &message,
  ...
  if (rootQmlObject) {
    ...
    QMetaObject::invokeMethod(
        rootQmlObject, "_showMessageDialog", Q_RETURN_ARG(QVariant, varReturn),
...
void QGCApplication::showRebootAppMessage(const QString &message,
  ...
  if (previousTime.isValid() &&
      (previousTime.msecsTo(currentTime) < (60 * 1000 * 2))) {
...
void QGCApplication::_checkForNewVersion() {
  ...
  const QString versionCheckFile =
      QGCCorePlugin::instance()->stableVersionCheckFileUrl();
  if (!versionCheckFile.isEmpty()) {
    ...
    if (!download->start(versionCheckFile)) {
      qCDebug(QGCApplicationLog)
          << "Download QGC stable version failed to start"
...
void QGCApplication::_qgcCurrentStableVersionDownloadComplete(
    bool success, const QString &localFile, const QString &errorMsg) {
  if (success) {
...
int QGCApplication::CompressedSignalList::_signalIndex(
    const QMetaMethod &method) {
  if (method.methodType() != QMetaMethod::Signal) {
    qCWarning(QGCApplicationLog) << "Internal error:" << Q_FUNC_INFO
...
void QGCApplication::CompressedSignalList::remove(const QMetaMethod &method) {
  ...
  if (signalIndex != -1 && _signalMap.contains(metaObject) &&
      _signalMap[metaObject].contains(signalIndex)) {
...
bool QGCApplication::CompressedSignalList::contains(
    const QMetaObject *metaObject, int signalIndex) {
  return _signalMap.contains(metaObject) &&
         _signalMap[metaObject].contains(signalIndex);
}
...
bool QGCApplication::compressEvent(QEvent *event, QObject *receiver,
  ...
  if (!mce->sender() || !_compressedSignals.contains(
                            mce->sender()->metaObject(), mce->signalId())) {
    return QGuiApplication::compressEvent(event, receiver, postedEvents);
  }

  for (QPostEventList::iterator it = postedEvents->begin();
       it != postedEvents->end(); ++it) {
    QPostEvent &cur = *it;
    if (cur.receiver != receiver || cur.event == 0 ||
        cur.event->type() != event->type()) {
      continue;
    }
    const QMetaCallEvent *cur_mce = static_cast<QMetaCallEvent *>(cur.event);
    if (cur_mce->sender() != mce->sender() ||
        cur_mce->signalId() != mce->signalId() || cur_mce->id() != mce->id()) {
      continue;
    }
    /* Keep The Newest Call */
    // We can't merely qSwap the existing posted event with the new one, since
    // QEvent keeps track of whether it has been posted. Deletion of a formerly
    // posted event takes the posted event list mutex and does a useless search
    // of the posted event list upon deletion. We thus clear the QEvent::posted
    // flag before deletion.
    struct EventHelper : private QEvent {
      static void clearPostedFlag(QEvent *ev) {
        (&static_cast<EventHelper *>(ev)->t)[1] &=
            ~0x8001; // Hack to clear QEvent::posted
      }
    };
...
bool QGCApplication::event(QEvent *e) {
  if (e->type() == QEvent::Quit) {
    ...
    // On OSX if the user selects Quit from the menu (or Command-Q) the
    // ApplicationWindow does not signal closing. Instead you get a Quit event
    // here only. This in turn causes the standard QGC shutdown sequence to not
    // run. So in this case we close the window ourselves such that the signal
    // is sent and the normal shutdown sequence runs.
    const bool forceClose = _mainRootWindow->property("_forceClose").toBool();
    qCDebug(QGCApplicationLog) << "Quit event" << forceClose;
    // forceClose
    //  true:   Standard QGC shutdown sequence is complete. Let the app quit
    //  normally by falling through to the base class processing. false:  QGC
    //  shutdown sequence has not been run yet. Don't let this event close the
    //  app yet. Close the main window to kick off the normal shutdown.
    if (!forceClose) {
      //
...
QGCImageProvider *QGCApplication::qgcImageProvider() {
  return dynamic_cast<QGCImageProvider *>(
  ...
  if (_runningUnitTests || _simpleBootTest) {
    ...
    if (settingsAppDir.exists()) {
      if (settingsAppDir.removeRecursively()) {
        qCDebug(QGCApplicationLog) << "Removed test run settings directory:"
                                   << settingsAppDir.absolutePath();
      } else {
        qCWarning(QGCApplicationLog)
            << "Failed to remove test run settings directory:"
            << settingsAppDir.absolutePath();
      }
    }

    QDir appDir(SettingsManager::instance()
                    ->appSettings()
                    ->savePath()
                    ->rawValue()
                    .toString());
    if (appDir.exists()) {
      if (appDir.removeRecursively()) {
        ...
        qCWarning(QGCApplicationLog)
            << "Failed to remove test run app data directory:"
            << appDir.absolutePath();
      }
    }
  }

  // This is bad, but currently qobject inheritances are incorrect and cause
  // crashes on exit without
